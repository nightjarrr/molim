...
def _prepare_folder(args, folder, index, count):
    def __ends_with(f, ext):
        return f.lower().endswith(ext.lower())

    _print.folder(f"\n[{index + 1}/{count}] Processing folder: {folder}")

    file_paths = [os.path.join(folder, f) for f in os.listdir(folder)]
    files = [
        f for f in file_paths if os.path.isfile(f) and __ends_with(f, args.video_files)
    ]
    ...
    for f in files:
        ...
        if not args.overwrite_processed and __ends_with(f, processed_mask):
            ...
            if size < args.greater_than:
                _print.filedetails(
                    f"- Skipping {f}({_byte_to_human(size)}) since it is smaller than {_byte_to_human(args.greater_than)}."
                )
                skipped += 1
            else:
                result.append(f)
...
def _process_file(args, folder, file, index, count):
    ...
    if args.video_files != ".mp4":
        ...
        _print.filedetails(
            "Extended ffmpeg params with audio codec to handle conversion."
        )

    # Output file
    filename = pathlib.Path(file).stem
    output_file = os.path.join(
        folder, f"{filename}.{args.suffix}{args.output_video_files}"
    )
    ...
    stat.output_file_size = (
        os.path.getsize(output_file) if not args.dry_run else stat.original_size
    )

    # original file post processing
    if args.originals == "move":
        destination = os.path.join(
            folder, args.originals_folder, os.path.basename(file)
        )
        ...
        _print.filedetails("Deleting original file.")
        if not args.dry_run:
            os.remove(file)
    ...
    _print.filedetails(
        f"New size: {_byte_to_human(stat.output_file_size)}, ({stat.delta_percent:.1f}% of original, saved {_byte_to_human(stat.delta_size)})"
    )
...
def _display_folder_stats(folder, elapsed, files_stats):
    ...
    _print.folder(
        f"\tProcessed {len(files_stats)} files ({_byte_to_human(old_total_size)} in total) in {timedelta(seconds=int(elapsed))}"
    )
    ...
    _print.folder(
        f"\tNew total size: {_byte_to_human(new_total_size)}, ({delta_percent:.1f}% of original, saved {_byte_to_human(delta_size)})\n"
    )
...
def _display_total_stats(elapsed, folders_count, files_stats):
    if files_stats:
        old_total_size = sum([stat.original_size for stat in files_stats])
        _print.totals(
            f"Processed {len(files_stats)} files in {folders_count} folder(s) ({_byte_to_human(old_total_size)}  in total) in {timedelta(seconds=int(elapsed))}"
        )
        new_total_size = sum([stat.output_file_size for stat in files_stats])
        delta_size = old_total_size - new_total_size
        delta_percent = 100 * float(new_total_size) / old_total_size
...
def main():
    ...
    parser.add_argument(
        "-c", "--codec", default=CODEC, help="Codec to use for processing."
    )
    parser.add_argument(
        "-r", "--rate", default=RATE, type=int, help="Processing compression rate."
    )
    parser.add_argument(
        "-o",
        "--originals",
    ...
    parser.add_argument(
        "-f",
        "--originals-folder",
        default=ORIGINALS_FOLDER,
        help="Subfolder to save originals into.",
    )
    parser.add_argument(
        "--display-verbosity",
        type=int,
        choices=range(1, 6),
        default=3,
        help="Verbosity of processing progress display.",
    )
    parser.add_argument(
        "--ffmpeg-report",
        default=False,
